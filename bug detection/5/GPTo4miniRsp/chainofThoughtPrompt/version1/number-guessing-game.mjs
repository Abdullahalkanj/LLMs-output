...
async function getHint() {
  ...
  const hints = [
    ...
    `The number is between ${Math.max(range[0], gameState.secretNumber - 10)} and ` +
      `${Math.min(range[1], gameState.secretNumber + 10)}.`,
    `The sum of its digits is ${
      String(gameState.secretNumber)
        .split('')
...
Round ${gameState.currentRound} - ${gameState.remainingGuesses} guess(es) left
Enter your guess (${difficultySettings[gameState.difficulty].range[0]}–` +
      ...
      if (
        ...
        console.log(colors.error(
          ...
          `${difficultySettings[gameState.difficulty].range[0]} and ` +
          `${difficultySettings[gameState.difficulty].range[1]}!`
        ));
        continue;
      }
