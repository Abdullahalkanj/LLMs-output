...
describe('Bug Detection in number-guessing-game.mjs', () => {
  ...
  expect(fnString).to.match(/function askQuestion/);
  expect(fnString).to.match(/\.trim\(\)/);
  expect(fnString).to.match(/rl\.question/);



  // Ensure it uses rl.question with 'prompt' (not any hardcoded wrong var)
  expect(fnString).to.match(/rl\.question\s*\(\s*prompt/);


  // Ensure correct return logic
  expect(fnString).to.match(/(return|resolve)\s*\(?.*\.trim\(\)/);
  ...
  it('selectDifficulty should validate 1, 2, or 3', () => {
    ...
    expect(code).to.match(/\(1\s*[-–]\s*3\)/);            // prompt should match
  });
  ...
  it('main should only replay if user says "y"', () => {
    ...
    /again(\.trim\(\))?\.toLowerCase\(\)\s*(===\s*['"]y['"]|\.startsWith\(['"]y['"]\))/);
